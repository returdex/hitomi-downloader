...
async testEhentaiConnectivity(cookie: string, favoritesUrl: string) : Promise<Result<EhentaiConnectivityResult, CommandError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("test_ehentai_connectivity", { cookie, favoritesUrl }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type EhentaiConnectivityResult = { checks: EhentaiConnectivityCheck[]; summary: string }
export type EhentaiConnectivityCheck = { label: string; url: string; statusCode: number; statusText: string; foundCount: number; bodyHint: string }
